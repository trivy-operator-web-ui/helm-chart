...
helm install \
  ...
  oci://ghcr.io/trivy-operator-web-ui/chart:0.3.0
```
...
| operator.username | string | `""` | Username for authentication. The same one to use on the frontend login page. |
| operator.password | string | `""` | Password for authentication. The same one to use on the frontend login page. |
